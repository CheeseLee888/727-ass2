...
# part III
from student_code import default_attack, default_visfunction
...
def main(args):
    ...
    # set up visualizer
    if args.vis:
        visualizer = default_visfunction
    else:
        visualizer = None
        ...
        for i, (input, target) in enumerate(val_loader):
            ...
            if i % args.print_freq == 0:
                ...
                if args.vis and args.evaluate:
                    vis_output = visualizer(input)
                    writer.add_image("Image/Image", vis_output, i)
                if args.vis and args.attack:
                    vis_input = visualizer(input)
                    vis_adv_input = visualizer(adv_input)
