...
class CustomConv2DFunction(Function):
    ...
    def forward(ctx, input_feats, weight, bias, stride=1, padding=0):
        ...
        ########################################################################
        # My Code
        output_height = math.floor((ctx.input_height+2*padding-kernel_size)/stride)+1
        output_width = math.floor((ctx.input_width+2*padding-kernel_size)/stride)+1

        unfolded_input = unfold(input_feats, kernel_size=kernel_size, padding=padding, stride=stride)
        reshaped_input = unfolded_input.permute(0, 2, 1)
        reshaped_weight = weight.reshape(weight.size(0),-1)
        linear_comb = torch.torch.einsum('nmc,oc->nmo', reshaped_input, reshaped_weight)
        if bias is not None:
            linear_comb += bias.unsqueeze(0).unsqueeze(0)
        output = linear_comb.permute(0, 2, 1).reshape(linear_comb.size(0), linear_comb.size(2), output_height, output_width)
    ...
    def backward(ctx, grad_output):
        ...
        # Gradients on Weights (Convolution on unfolded input with dL/dO)

        vectorized_grad_output = grad_output.reshape(grad_output.size(0), grad_output.size(1), -1)
        raw_grad_weight = torch.einsum("nml,ncl->cm", unfolded_input, vectorized_grad_output)
        grad_weight = raw_grad_weight.reshape(weight.shape)
        
        # Gradients on Input

        reshaped_weight = weight.reshape(weight.size(0), -1)
        raw_grad_input = torch.einsum("cm,ncl->nml",reshaped_weight, vectorized_grad_output)
        grad_input = fold(raw_grad_input, output_size=[input_height, input_width], kernel_size=kernel_size, stride=stride, padding=padding)
